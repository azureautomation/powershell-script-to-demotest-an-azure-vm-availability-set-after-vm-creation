...
# Script to Demo/Test an Azure VM Availability Set after VM creation
# Sam Boutros - 14 December, 2015 - v1.0

#region Input
$SubscriptionName = 'Sam Test 1' 
$Prefix           = 'DemoAvSet3' # Max 11 characters - used for VM1 name, VM2 name, Cloud Service name, Storage Account name, Availability Set name
...
1..2 | % {
    ...
    $VMNames += "$Prefix-VM$_"
    try {
        $VMConfig = New-AzureVMConfig -Name "$Prefix-VM$_" -InstanceSize Small -ImageName $VMImage.ImageName -ErrorAction Stop
...
# Consider status 'Not Ready' until all VMs' status is 'ReadyRole'
...
log 'Creating Availability Set',"$Prefix-AvailSet",'..' Cyan,Green,Cyan 
try {
    Get-AzureVM -Name $VMNames[0] -ServiceName $CloudServiceName |
        Set-AzureAvailabilitySet -AvailabilitySetName "$Prefix-AvailSet" -ErrorAction Stop |
            Update-AzureVM
    log 'done' Green
} catch { log 'failed' Magenta; break }

# Wait for VM1 to reboot, up to 5 minutes
$BreakTime = (Get-Date) + (New-TimeSpan -Minutes 5)
$VMStatus = (Get-AzureVM | Where { $_.Name -eq $VMNames[0] }).status
log "Rebooting $($VMNames[0]).." Cyan -NoNewLine
while ($VMStatus -ne 'ReadyRole' -and (Get-Date) -lt $BreakTime) {
    Log '.' Cyan -NoNewLine
    $VMStatus = (Get-AzureVM | Where { $_.Name -eq $VMNames[0] }).status
    Start-Sleep -Seconds 5
}
...
# Add VM2 to availability set:
log 'Adding', $VMNames[1], 'to Availability Set',"$Prefix-AvailSet",'..' Cyan,Green,Cyan,Green,Cyan 
try {
    Get-AzureVM -Name $VMNames[1] -ServiceName $CloudServiceName |
        Set-AzureAvailabilitySet -AvailabilitySetName "$Prefix-AvailSet" -ErrorAction Stop |
            Update-AzureVM
    log 'done' Green
} catch { log 'failed' Magenta; break }

# Wait for VM2 to reboot
$VMStatus = (Get-AzureVM | Where { $_.Name -eq $VMNames[1] }).status
log "Rebooting $($VMNames[1]).." Cyan -NoNewLine
